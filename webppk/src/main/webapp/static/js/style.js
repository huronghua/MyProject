...
$(function(){
    var h=$(window).height();
	var w=$(window).width();
	$(".spage").css("height",h-105+"px");
	$(".spage").css("width",w-100+"px");
});


$(document).ready(function(){
  $(".clockpropop").click(function(){
    $(".pom").fadeIn();
  });
});
$(function(){
  $(".close").click(function(){
	  $('#pom', window.parent.document).fadeOut(100);
   });
});
$(document).ready(function(){
  $(".close").click(function(){
...
/* ---------- 部件 -----------------*/
$(document).ready(function(){
	var $tab_li = $('.partsbox_tab li');
	$tab_li.click(function(){
		$(this).addClass('active').siblings().removeClass('active');
		var index = $tab_li.index(this);
		$('.partsbox_cot .partsbox_group').eq(index).show().siblings().hide();
	});	
});
...
/*------------------表格 --------------------------- */
 function add_tablerow(){
	 $(".settablebox").append($("tr").last().clone(true));
};
function add_tablecol(){
   $(".settablebox tr:first").append($("tr:first th:last").clone(true));
	$(".settablebox .tablerow").append($("td:last").clone(true));
   
};
$(function(){
	//var  settable_value=$(".settable_h").val();
    //var settable_length=settable_value.length;
   $('.settable_h').bind('input propertychange', function() {
    $(this).parents('.settablebox tr th').height($(this).val());
   });
   $('.settable_w').bind('input propertychange', function() {
    $(this).parents('.settablebox tr th').width($(this).val()+"%");
   });

})
$(function(){
	$(".settablebox .delhang").click(function(){
